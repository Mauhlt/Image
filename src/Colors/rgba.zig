...
test "RGBA" {
    ...
    try std.testing.expectEqualDeep(rgba, RGBA{ .r = data[0], .g = data[1], .b = data[2], .a = data[3] });

    const rgba2: RGBA = .initOrder(&data, .abgr);
    try std.testing.expectEqualDeep(rgba2, RGBA{ .r = data[3], .g = data[2], .b = data[1], .a = data[0] });
